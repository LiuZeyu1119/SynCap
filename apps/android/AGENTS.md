...
## SynCap Product Decisions

- The app brand is `SynCap Studio`; `RoboBaton-4P` is a connected device model, not the product name.
- Preserve the approved obsidian-black, champagne-gold, titanium-gray and restrained emerald health-state visual system.
...
- Use `BottomSheet` for phone-scoped sheets. Its props are `open`, `onOpenChange`, `title`, optional `description`, optional `snap`, optional `contentClassName`, and `children`; it renders through the phone screen portal and dismisses the keyboard before opening.
