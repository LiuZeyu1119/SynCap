...
function usesSystemKeyboard() {
  if (typeof document === "undefined") return false;
  return Boolean(document.documentElement.dataset.nativePlatform || document.documentElement.dataset.desktopShell);
}
  ...
  return {
    onPointerDown: (event: ReactPointerEvent<HTMLElement>) => {
      ...
      if (
        ...
        event.target.closest('button, select, a, [role="button"]')
      ) {
        return;
      }
...
export function KeyboardInput(props: KeyboardInputProps) {
  ...
  return (
    <input
      ...
      ref={ref}
      onFocus={(event) => {
        if (!usesSystemKeyboard()) keyboard.show(event.currentTarget);
